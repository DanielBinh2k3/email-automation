...
const EmailEditor = () => {
    ...
    const autoSuggest = async () => {
        ...
        setIsGenerating(true); // Use isGenerating for the improvement step
        try {
      ...
      `;
            ...
            toast.success("Email đã được cải thiện thành công!"); // Show success toast
            closeModal(); // Close modal on success
        } catch (error) {
            console.error('Lỗi khi tự động gợi ý email:', error);
            toast.error(`Lỗi: ${error.message}`); // Show error toast
